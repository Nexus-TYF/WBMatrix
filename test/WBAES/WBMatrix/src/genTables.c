...
void computeTables (u8 expandedKey[176])
{
  ...
  {
    for (int k = 0; k < 4; k++) {
      genMatpairM32(&mb_32x32[i][k][0],&mb_32x32[i][k][1]);
    }

    for (int k = 0; k < 16; k++) {
      genMatpairM8(&mb_8x8[i][k][0],&mb_8x8[i][k][1]);
    }
  }
  ...
  for (int i = 0; i < 10; i++)
  {
    ...
    for (int j = 0; j < 16; j++)
    {
      for (int x = 0; x < 256; x++)
      {
        mixBijIn[i][j][x] = MatMulNumM8(mb_8x8[i][j][0], x);
      }
      for (int x = 0; x < 256; x++)
      {
        u32 tmp = x;
        ...
        if ((i > 0) && (i < 9)) { 
          tmp = MatMulNumM8(mb_8x8[i-1][j][1], x);       
        }
  ...
  for (int i = 0; i < 10; i++)
  {
    for (int j = 0; j < 4; j++)
    {
        for (int x = 0; x < 256; x++)
        {
          if (i < 9)
          {
            mixBijIn[i][4*j + 0][x] = MatMulNumM8(mb_8x8[i][4*j + 0][0], x);
            mixBijIn[i][4*j + 1][x] = MatMulNumM8(mb_8x8[i][4*j + 1][0], x);
            mixBijIn[i][4*j + 2][x] = MatMulNumM8(mb_8x8[i][4*j + 2][0], x);
            mixBijIn[i][4*j + 3][x] = MatMulNumM8(mb_8x8[i][4*j + 3][0], x);
        ...
        for (int x = 0; x < 256; x++)
        {
          if (i < 9)
          {
            vAA.V = TyiBoxes[i][4*j + 0][x];
            vBB.V = TyiBoxes[i][4*j + 1][x];
            vCC.V = TyiBoxes[i][4*j + 2][x];
            vDD.V = TyiBoxes[i][4*j + 3][x];

            TyiBoxes[i][4*j + 0][x] = MatMulNumM32(mb_32x32[i][j][0], vAA.V);
            TyiBoxes[i][4*j + 1][x] = MatMulNumM32(mb_32x32[i][j][0], vBB.V);
            TyiBoxes[i][4*j + 2][x] = MatMulNumM32(mb_32x32[i][j][0], vCC.V);
            TyiBoxes[i][4*j + 3][x] = MatMulNumM32(mb_32x32[i][j][0], vDD.V);
            ...
            aa = MatMulNumM32(mb_32x32[i][j][1], vAA.V);
            bb = MatMulNumM32(mb_32x32[i][j][1], vBB.V);
            cc = MatMulNumM32(mb_32x32[i][j][1], vCC.V);
            dd = MatMulNumM32(mb_32x32[i][j][1], vDD.V);
